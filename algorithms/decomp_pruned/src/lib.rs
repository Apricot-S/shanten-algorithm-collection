...
fn cut_meld(hand: &mut TileCounts, block_counts: &mut BlockCounts, min_shanten: &mut i8, i: usize) {
    if i >= NUM_TILE_TYPES {
        let lower_bound = block_counts.lower_bound();
        cut_meld_cand(hand, block_counts, min_shanten, 0, lower_bound);
        return;
    }
    ...
    if hand[i] >= 3 {
        ...
        hand[i] -= 3;
        cut_meld(hand, block_counts, min_shanten, i);
        hand[i] += 3;
        block_counts.melds -= 1;
    }
    ...
    cut_meld(hand, block_counts, min_shanten, i + 1);
}
...
fn cut_meld_cand(
    ...
    if block_counts.total_meld_blocks() < MAX_NUM_BLOCKS {
        ...
        if hand[i] == 2 {
            ...
            hand[i] -= 2;
            cut_meld_cand(hand, block_counts, min_shanten, i, lower_bound);
            hand[i] += 2;
            block_counts.meld_candidates -= 1;
        }
    ...
    cut_meld_cand(hand, block_counts, min_shanten, i + 1, lower_bound);
}
    ...
    fn calculate_shanten(&self, hand: &TileCounts) -> i8 {
        ...
        for i in 0..NUM_TILE_TYPES {
            if hand_clone[i] >= 2 {
                ...
                hand_clone[i] -= 2;
                cut_meld(&mut hand_clone, &mut block_counts, &mut min_shanten, 0);
                hand_clone[i] += 2;
                block_counts.pairs -= 1;
            }
        }
