...
fn cut_meld_cand(
    ...
    if num_blocks.get_num_blocks() < 4 {
        // pair (triplet candidate)
        if hand[n] >= 2 {
            num_blocks.num_meld_cand += 1;
            hand[n] -= 2;
            cut_meld_cand(hand, num_blocks, min_shanten, n);
            hand[n] += 2;
            num_blocks.num_meld_cand -= 1;
        }
    ...
    cut_meld_cand(hand, num_blocks, min_shanten, n + 1);
}
...
impl ShantenCalculator for Decomp {
    ...
    fn calculate_shanten(&self, hand: &TileCounts) -> i8 {
        ...
        for n in 0..(NUM_TILE_TYPE as usize) {
            if hand_clone[n] >= 2 {
